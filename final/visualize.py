...
class UniversalDatasetViewer:
    ...
    def __init__(self, dataset_path, dataset_format="yolo", batch_size=4, class_names=None, colors=None):
        ...
        Args:
            ...
            dataset_format: 'yolo', 'coco', 'voc', or 'directory' ( all formats supported by datmuro)
        ...
        # Setup figure
        cols = int(np.ceil(self.batch_size/2))
        rows = 2 if self.batch_size > 1 else 1
        self.fig, self.axs = plt.subplots(rows, cols, figsize=(7*cols, 5*rows))
    ...
    def _load_datumaro(self):
        ...
        try:
            ...
            print(f"Failed to load dataset via Datumaro: {self.error_message}")
            traceback.print_exc() #for logging
            print("Falling back to directory mode.")
            self.dataset_format = "directory"
            self._load_directory()
            # self.items = []

    def _load_class_names(self):
        """Extract class names from dataset annotations' files."""
        max_label = 0
        for item in self.items:
            for annotations in getattr(item, 'annotations', []):
                if hasattr(annotations, 'label'):
                    max_label = max(max_label, annotations.label)
...
if __name__ == "__main__":
    ...
    DATASET_FORMAT = "yolo"  # options: "yolo", "coco", "voc", "directory", etc.
    BATCH_SIZE = 4
    CLASS_NAMES = None
    CUSTOM_COLORS = None
